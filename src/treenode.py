...
from cnn_model import ConvNetAnn
import prepare
...
class TreeNode():
    def __init__(self,with_comb,mods,n_input, n_hidden, num_class, model_type='neural'):
        ...
        ######## combination of models only #####################                                                                                                                                                                                              
        self.with_combination = with_comb
        self.models_list = mods
         
        
        if (self.with_combination==True):
                # set the model type that the model_picker chose                                                                                                                                                                                               
                model_type = model_picker(self.models_list)

        ##################################### each tree node is a model of type 'neural' or 'linear' depending on given parameters ##############
        if model_type == 'neural':
            ...
            # note : should customize n_hidden,num_class
            # SOS :  RESHAPE N_INPUT = INPUT_DIM = 784 INTO ---> 28,28 FOR CNN

            #self.n_input = n_input.reshape(-1,)
            self.model = ConvNetAnn(n_input=n_input, num_class=num_class,n_hidden=self.n_hidden,opt=None).cuda()
            # ------APPARENTLY THE MODEL NEEDS TO BE DOUBLE AS WELL --------------#
            # attention here
            self.model=self.model.double() # or float()
            #---------------------------------------------------------------------#
            #should add printing model summary
            #summary(self.model,(1,28,28))
            #print(n_input.shape)
        else :
            raise ValueError('model_type must be either neural or linear or cnn')
    ...
    ############################## produce bins probabilities --> confidence_scores ######
    def infer(self, X, return_confidence_scores=False):
        ...
        if(self.model_type=='cnn'):
            #print(f'\nBefore inference it has shape : {X_scaled.shape}\n')
            X_scaled = torch.reshape(X_scaled,(-1,1,28,28))

            # cast both input and model to float
            X_scaled=X_scaled.float() # ? error mismatch !!
            self.model.float()
        ############
    ...
    def train_model(self, X, Y, input_weights, ensemble_model_index, crit, data, batch_size=4096, iterations=5, lr=1e-3, use_generated_batches=False, use_new_knn_matrix=False, models_path = None):
        ...
        ######################## reshape X for cnn #######################################
        if(self.model_type=='cnn'):
            X_scaled_2 =torch.reshape(X_scaled,(-1,28,28))
            print(f'\n[X_scaled_2] has shape : {X_scaled_2.shape}')
        ##################################################################################
        ...
        if use_generated_batches and iterations > 0:
            ...
            for k in range(int(n / batch_size) + 1):
                ...
                ############## add reshape of batch for cnn ####################
                if (self.model_type == 'cnn'):
                    X_batch2=torch.reshape(X_batch,(batch_size,1,28,28))
                #if(self.model_type=='cnn'):
                ##############################################
        ...
        for ep in range(1, iterations + 1):
            ...
            for k in range(int(n / batch_size) + 1):
                ...
                if batch_size < n and not use_generated_batches:
                    ...
                    ############ create x batch for cnn model #############
                    if(self.model_type=='cnn'):
                        x_batch_2d = torch.reshape(X_batch,(batch_size,28,28))
                    #######################################################
                    ...
                    if collect_knns:
                        ...
                        ########## cnn ##################
                        if(self.model_type=='cnn'):
                            knns2 = torch.reshape(knns,(-1,28,28))
                            #print(f'\n[knns2] (reshaped) have shape {knns2.shape}')
                        #####################################
                        ...
                        ############## cnn ##################
                        if(self.model_type=='cnn'):
                            X_knn_batch = torch.cat((x_batch_2d,knns2),0)
                            # add 4th dimension in dim=1 so from (N+M,28,28) -->
                            #it now has the shape : (N+M,1,28,28)=(batches,channels,width,height) FOR THE CNN
                            X_knn_batch = X_knn_batch.unsqueeze(1)
                        #######################################
                        ...
                        ################## cnn ####################################
                        if(self.model_type == 'cnn'):
                            X_batch = torch.reshape(X_batch,(-1,28,28))
                            X_batch = X_batch.unsqueeze(1)
                        ################### get confidence scores , predicted bins ###########
                ...
                ############################# back propagation ############
                running_b_top_size_bound = batch_size #org_n=running_b_top_size_bound
                (loss, diff_sum, b, _, _ ) = crit(y_pred, Y_batch, input_weights_batch, running_b_top_size_bound)
                del Y_batch
