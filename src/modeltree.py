...
#### class for tree---> Hierararchical partitioning ###
class ModelTree():
    def __init__(self,with_comb,mods, n_input, branching, levels, n_classes, n_hidden_params, model_type='neural'):
        ...
        if(self.with_combination==False):
        ...
        ############# node of 'TreeNode' type ##########
        self.root = TreeNode(self.with_combination,self.models_list,n_input, n_hidden_params, root_classes, model_type)
        ...
        while(len(q) > 0):
            ...
            for b in range(branching):
                ...
                if model.level >= levels - 1:
                    ...
                    child = TreeNode(self.with_combination,self.models_list,n_input=self.n_input, n_hidden=self.n_hidden_params, num_class=n_classes_per_leaf, model_type=self.model_type).to(self.secondary_device)
                else:
                    child = TreeNode(self.with_combination,self.models_list,n_input=self.n_input, n_hidden=self.n_hidden_params, num_class=self.branching, model_type=self.model_type).to(self.secondary_device)
                child.parent = model
    ...
    def train_model_tree_or_infer(self, X, Y, Q, input_weights, crit, data, train_mode=True,batch_size=4096,iterations=5,lr=1e-3, bin_count=1, model_loaded_from_file=False, test_X=None, test_Y=None, tree_index=0, models_path=None):
        ...
        while(len(q) > 0): # BFS
            ...
            if train_mode:
                ...
                if start:
                   plt.plot(losses, marker="x", label="no of models: " + str(tree_index + 1))
    ...
    def infer_new(self, Q, batch_size=4096, bin_count=1):
        ...
        while(len(q) > 0): # BFS
            model = q.pop(0)
            assert isinstance(model, TreeNode)
            # check if query point of type TreeNode
