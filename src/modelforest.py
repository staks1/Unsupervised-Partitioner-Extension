...
class ModelForest():
    def __init__(self, with_comb,mods,n_trees, n_input, branching, levels, n_classes, n_hidden_params, model_type='neural') -> None:
    ...
    def build_forest(self, load_from_file=False, data=None, models_path=None):
        ...
        for i in range(self.n_trees):
            ...
            tree = ModelTree(self.with_combination,self.models_list,self.n_input, self.branching, self.levels, self.n_classes, self.n_hidden_params,  model_type=self.model_type)
            tree.build_tree_from_root(load_from_file=load_from_file, data=data, models_path=models_path)
            self.trees.append(tree)
            self.data = data
        self.load_from_file = load_from_file

    ################################## function to train the forest #######################
        ...
        for (i, model_tree) in enumerate(self.trees):
            ...
            # for j in range(max(i-1, 0), i):  # only the prev model, except for 0th model
                ...
                for k in range(0, n, batch_size):
                    ...
                    # the model returns the confidence scores for each bin
                    ...
                    (_, _, _, _, current_weights) = crit(y_pred_assigned_bins.flatten(), Y_batch, input_weights_batch, calculate_booster_weights=True, n_bins=self.n_classes)
                    # set confidence_scores=scores not inside crit(...) but here just for testing purposes
                    confidence_scores=scores
